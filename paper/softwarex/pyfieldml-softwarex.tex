...
\begin{figure}[!h]
\centering
\includegraphics[width=0.80\textwidth]{figures/fig3_evaluator_graph.png}
\caption{Evaluator graph of the bundled \texttt{rectus\_femoris} dataset. Nodes are coloured by evaluator subtype (parameter / external / aggregate / reference / piecewise / argument / constant) following the FieldML 0.5 taxonomy. Edges are named dependencies: the \texttt{coordinates} parameter depends on its \texttt{coordinates.connectivity} ensemble, which in turn binds into the element topology via an \texttt{ExternalEvaluator} basis.}
\label{fig:graph}
\end{figure}

\subsection{Example 2: FEM solve via scikit-fem}

A FieldML document can be handed directly to scikit-fem \cite{scikit_fem} for PDE assembly through the \texttt{pyfieldml.interop.scikit\_fem} bridge, which returns a \texttt{(Mesh, Basis)} pair. The Poisson problem $-\Delta u = 1$ on a unit cube, for instance, reduces to a standard scikit-fem boilerplate once the mesh has been obtained from FieldML (Fig.~\ref{fig:fem}). The pattern generalises to any FieldML mesh whose basis is supported by scikit-fem, including the triangular-surface BodyParts3D bones. Crucially, neither the user nor the solver is aware that the mesh began life as a FieldML 0.5 evaluator graph---the bridge is transparent.

\begin{figure}[!h]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig5_fem_solution.png}
\caption{FEM workflow starting from a FieldML mesh. Left: a refined tetrahedral mesh derived from the bundled \texttt{unit\_cube} via the \texttt{pyfieldml $\rightarrow$ scikit-fem} bridge. Right: Poisson solution $-\Delta u = 1$ with zero Dirichlet boundary, rendered on three orthogonal slices.}
\label{fig:fem}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig6_msk_assembly.png}
\caption{Clinical musculoskeletal assembly rendered from four bundled BodyParts3D \cite{bodyparts3d} datasets (left hip, left femur, left tibia, lumbar vertebra L3). The scene is assembled in roughly ten lines of Python on top of \texttt{pyfieldml.datasets}. BodyParts3D data is \copyright{} The Database Center for Life Science (DBCLS), licensed under CC-BY-SA 2.1 JP.}
\label{fig:msk}
\end{figure}

Beyond assembly, the Hermite basis of the evaluation engine enables smooth-deformation studies. Fig.~\ref{fig:hermite} shows a two-node cubic-Hermite line element, the simplest setting in which the C1-continuous Hermite basis is visible, loaded as a tip-force cantilever. pyfieldml samples the deflection at 41 xi points; the resulting curve is C1-continuous by construction, matching the theoretical Euler-Bernoulli profile. The same Hermite machinery is what enables smooth heart-wall geometries in Physiome-Project cardiac models \cite{nash_hunter_2001}.

\begin{figure}[!h]
\centering
\includegraphics[width=0.98\textwidth]{figures/fig4_hermite_bending.png}
\caption{Cantilever bending with the \texttt{pyfieldml} cubic-Hermite basis. Left: undeformed two-node Hermite line element. Right: tip-loaded deflection, sampled at 41 parametric ($\xi$) points via \texttt{Field.evaluate}. The profile is C1-continuous by basis construction.}
\label{fig:hermite}
\end{figure}
